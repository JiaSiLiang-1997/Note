...
![JVM运行时数据区域.297a28f3](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/JVM%E8%BF%90%E8%A1%8C%E6%97%B6%E6%95%B0%E6%8D%AE%E5%8C%BA%E5%9F%9F.297a28f3.png)

**JDK 1.8 ：**

![Java运行时数据区域JDK1.8.ce907461](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/Java%E8%BF%90%E8%A1%8C%E6%97%B6%E6%95%B0%E6%8D%AE%E5%8C%BA%E5%9F%9FJDK1.8.ce907461.png)
...
![《深入理解虚拟机》第三版的第2章-虚拟机栈.59d0736f](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/%E3%80%8A%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3%E8%99%9A%E6%8B%9F%E6%9C%BA%E3%80%8B%E7%AC%AC%E4%B8%89%E7%89%88%E7%9A%84%E7%AC%AC2%E7%AB%A0-%E8%99%9A%E6%8B%9F%E6%9C%BA%E6%A0%88.59d0736f.png)
...
![JVM堆内存结构-JDK7.7d9166eb](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/JVM%E5%A0%86%E5%86%85%E5%AD%98%E7%BB%93%E6%9E%84-JDK7.7d9166eb.png)

JDK 8 版本之后方法区（HotSpot 的永久代）被彻底移除了（JDK1.7 就已经开始了），取而代之是元空间，元空间使用的是直接内存。

![JVM堆内存结构-jdk8.919b9959](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/JVM%E5%A0%86%E5%86%85%E5%AD%98%E7%BB%93%E6%9E%84-jdk8.919b9959.png)
...
JDK1.4 中新加入的 **NIO(New Input/Output) 类**，引入了一种基于**通道（Channel）**与**缓存区（Buffer）**的 I/O 方式，它可以直接使用 Native 函数库直接分配堆外内存，然后通过一个存储在 Java 堆中的 **DirectByteBuffer 对象**作为这块内存的引用进行操作。这样就能在一些场景中显著提高性能，因为**避免了在 Java 堆和 Native 堆之间来回复制数据**。
...
<img src="https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220207163827848.png" alt="image-20220207163827848" style="zoom:80%;" />
...
![image-20220207163902792](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220207163902792.png)

**指针碰撞：**用过的内存全部整合到一边，没有用过的内存放在另一边，中间有一个分界值指针，只需要向着没用过的内存方向将该指针移动对象内存大小位置即可。
...
- **TLAB：** 为每一个线程预先在 Eden 区分配一块儿内存，JVM 在给线程中的对象分配内存时，首先在 TLAB 分配，当对象大于 TLAB 中的剩余内存或 TLAB 的内存已用尽时，再采用CAS 进行内存分配

- **CAS+失败重试：** CAS 是乐观锁的一种实现方式。所谓乐观锁就是，每次不加锁而是假设没有冲突而去完成某项操作，如果因为冲突失败就重试，直到成功为止。**虚拟机采用 CAS 配上失败重试的方式保证更新操作的原子性。**

![JVM对象创建内存分配流程](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/JVM%E5%AF%B9%E8%B1%A1%E5%88%9B%E5%BB%BA%E5%86%85%E5%AD%98%E5%88%86%E9%85%8D%E6%B5%81%E7%A8%8B.png)

**栈上分配：**

栈上分配是java虚拟机提供的一种**优化技术**，基本思想是对于那些**线程私有的对象**（指的是不可能被其他线程访问的对象），可以将它们**打散分配**在**栈**上，而不是分配在堆上。分配在栈上的好处是可以在函数调用结束后**自行销毁**，而不需要垃圾回收器的介入，栈上分配速度快，提高系统性能。其局限性：栈空间小，对于大对象无法实现栈上分配。栈上分配两个技术基础：逃逸分析 + 标量替换
...
- 栈上分配速度快，提高系统性能同样的User的对象实例，分配100000000次，启用栈上分配，只需6ms，不启用，需要3S；

**为什么需要**TLAB？**

定义：TLAB，全称Thread Local Allocation Buffer, 即：线程本地分配缓存。这是一块**线程专用**的内存分配区域。TLAB占用的是eden区的空间。在TLAB启用的情况下（默认开启），JVM会为每一个线程分配一块TLAB区域。

目的：这是为了加速对象的分配。由于对象一般分配在堆上，而堆是线程共用的，因此可能会有多个线程在堆上申请空间，而每一次的对象分配都必须线程同步，会使分配的效率下降。考虑到对象分配几乎是Java中最常用的操作，因此JVM使用了TLAB这样的线程专有区域来避免多线程冲突，提高对象分配的效率。
...
![image-20220205220445719](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205220445719.png)

**Object对象内存布局分析**：

创建一个Object对象，查看其内存布局：

![image-20220205211421401](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205211421401.png)

- 前两个object header为markdown（8个字节）；
- 第三个object header为class pointer（4个字节，压缩后）；

如果关闭类型指针压缩（`-XX:-UseCompressedClassPointers`），其内存布局：

![image-20220205222315164](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205222315164.png)

- 前两个object header为markdown（8个字节）；
- 后两个object header为class pointer（8个字节）；
...
![image-20220205223943918](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205223943918.png)
...
![image-20220205223903984](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205223903984.png)
...
![image-20220205220614580](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205220614580.png)

- 前两个object header为markdown（8个字节）；

- 第三个object header为class pointer（4个字节，压缩后）；

- 第四个object header为数组长度标识；
...
- 对于对象头长度而言
  ...
  - -XX:-UseCompressedClassPointers：对象头中类型指针的压缩
  - **在《深入理解Java虚拟机（第二版）》中，说对象头是8字节或16字节，不知道是不是有误，自己的系统不是64bit，没有测试**
- 基本数据类型与对应包装类的选用

![img](https://images2015.cnblogs.com/blog/866881/201602/866881-20160203215449772-2071796524.jpg)

在实际使用中，我们会根据字节数较小的一方来选用基本数据类型还是使用其包装类。

- 在《[第二章 JVM内存分配](http://www.cnblogs.com/java-zhao/p/5180492.html) 》中说过，实际对象所占的内存大小在类加载完成后就可以知道了，具体是怎样计算的，可以参看这一篇文章http://www.importnew.com/14948.html
...
![image-20220205211002401](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205211002401.png)

2. **直接指针：** 如果使用直接指针访问，那么 Java 堆对象的布局中就必须考虑如何放置访问类型数据的相关信息，而 reference 中存储的直接就是对象的地址。

![image-20220205211019336](https://jsl1997.oss-cn-beijing.aliyuncs.com/note/image-20220205211019336.png)
